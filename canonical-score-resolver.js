...
(function(root,factory){
  ...
  function resolveLiveProjected({intradaySnapshots,targetDate,scoreVersion,symbols,now=new Date(),marketState=MARKET_STATES.STALE,maxAgeMinutes=75}={}){
    ...
    for(const symbol of expected){
      ...
      if(!row||row.status!=="SUCCESS"||!finiteScore(row.score)||String(row.market_as_of||"").startsWith(`${targetDate}T`)===false||row.freshness!=="FRESH")return liveUnavailable(LIVE_REASONS.INCOMPLETE_INTRADAY,marketState,snapshot);
