...
for (const symbol of research.SYMBOLS) {
  ...
  assert.equal(artifact.metadata.provenance.builder_sha256,
    research.sha(fs.readFileSync(path.join(ROOT, "scripts", "build_c4_research_history.js"), "utf8").replace(/\r\n/g, "\n")));
  assert.equal(artifact.metadata.provenance.weekly_j_helper_sha256,
    research.sha(fs.readFileSync(path.join(dir, "weekly_j_versions.js"), "utf8").replace(/\r\n/g, "\n")));
