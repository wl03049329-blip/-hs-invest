...
const css = fs.readFileSync("v62-tech.css","utf8") + fs.readFileSync("formal-black-gold.css","utf8");
...
for(const label of ["HS LEVERAGE","5D Crash Velocity","極端觸發門檻","距離極端觸發","目前策略狀態"])assert.match(html,new RegExp(label));
...
const visibleHomeOrder = ["hsDecisionRoom","homeSentiment","homeEtfBrief","homeLeverageBrief"];
let cursor = -1;
...
assert.match(css,/hsDecisionRoomGrid/);
assert.match(css,/@media\(max-width:430px\)[\s\S]*hsDecisionRoom/);
