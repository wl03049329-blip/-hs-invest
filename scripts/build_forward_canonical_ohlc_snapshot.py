...
import argparse, datetime as dt, hashlib, json, math, pathlib, subprocess, urllib.parse, urllib.request
...
def verify_producer_commit(value):
    if not isinstance(value,str) or not __import__("re").fullmatch(r"[0-9a-f]{40}",value): raise ValueError("FAIL_CLOSED_PRODUCER_COMMIT_SHA")
    try: committed=subprocess.check_output(["git","show",f"{value}:scripts/build_forward_canonical_ohlc_snapshot.py"],cwd=pathlib.Path(__file__).parents[1])
    except Exception as error: raise ValueError("FAIL_CLOSED_PRODUCER_COMMIT_UNAVAILABLE") from error
    if digest(committed)!=producer_hash(): raise ValueError("FAIL_CLOSED_PRODUCER_COMMIT_FILE_MISMATCH")
    return value
def fetch(symbol):
...
def rows_from_chart(chart,ticker,cutoff):
    ...
    return rows
def build(charts,cutoff,generated_at,producer_commit):
    if not isinstance(producer_commit,str) or not __import__("re").fullmatch(r"[0-9a-f]{40}",producer_commit): raise ValueError("FAIL_CLOSED_PRODUCER_COMMIT_SHA")
    items={}; provenance={}
    ...
    return dataset,{"snapshot_schema_version":SCHEMA_VERSION,"trading_date":cutoff,"generated_at":generated_at,"timezone":"Asia/Taipei","signal_policy":"STRICTLY_THROUGH_T_CLOSE","upstream_provider":"Yahoo Finance Chart API","upstream_endpoint_family":"query1.finance.yahoo.com/v8/finance/chart","etf_universe":list(UNIVERSE),"coverage":coverage,"producer_sha256":producer_hash(),"producer_commit_sha":producer_commit,"producer_schema_version":SCHEMA_VERSION,"adjustment_algorithm_version":ADJUSTMENT_ALGORITHM_VERSION,"repair_manifest_version":REPAIR_MANIFEST_VERSION,"repair_manifest":REPAIR,"source_provenance":provenance,"data_quality_status":"PASS","no_lookahead_validation":{"result":"PASS","rule":"all rows date <= trading_date"}}
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--trading-date",required=True); parser.add_argument("--output-root",required=True); parser.add_argument("--producer-commit",required=True); args=parser.parse_args(); dt.date.fromisoformat(args.trading_date)
    producer_commit=verify_producer_commit(args.producer_commit); generated=dt.datetime.now(dt.timezone.utc).isoformat(); charts={ticker:fetch(symbol) for ticker,symbol in UNIVERSE.items()}; dataset,manifest=build(charts,args.trading_date,generated,producer_commit); status,directory,manifest_hash=write_immutable(pathlib.Path(args.output_root),args.trading_date,dataset,manifest); print(json.dumps({"status":status,"path":str(directory),"dataset_sha256":manifest["dataset_sha256"] if "dataset_sha256" in manifest else digest(canonical(dataset)),"manifest_sha256":manifest_hash}))
