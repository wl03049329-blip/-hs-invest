...
class BackgroundAlertTests(unittest.TestCase):
    ...
    def test_16_new_finalized_sends_one_bundle_and_restart_dedups(self):
        with tempfile.TemporaryDirectory() as root:
            path=Path(root,"final.json");path.write_text(json.dumps(artifact([39])),encoding="utf-8");sent=[];store=PushSubscriptionStore(root);service=PushService(store,config=CONFIG,sender=lambda **kw:sent.append(kw));sid=service.subscribe(SUB,user_agent="Chrome")["subscription_id"];e=BackgroundAlertEvaluator(service,finalized_path=path);e.sync_rules(sid,PREF);path.write_text(json.dumps(artifact([39,40])),encoding="utf-8");self.assertEqual(e.evaluate_once()["push_success_count"],1);restarted=BackgroundAlertEvaluator(PushService(PushSubscriptionStore(root),config=CONFIG,sender=lambda **kw:sent.append(kw)),finalized_path=path);self.assertEqual(restarted.evaluate_once()["bundle_count"],0);self.assertEqual(len(sent),1)
