...
assert.match(html,/change>0\?`↑\$\{change\}`:change<0\?`↓/);
assert.match(html,/LONG_RANK_STORAGE_KEY/);
for(const field of ["current_score","previous_score","current_rank","previous_rank","slot_time"])assert.match(html,new RegExp(field));
...
assert.match(html,/minute>=30&&minute<=39/);
