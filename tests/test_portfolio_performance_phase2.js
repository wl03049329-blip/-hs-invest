...
check("UI declares forward-only history and adjusted 0050 source", () => { assert.match(ui,/FORWARD_SNAPSHOT_ONLY/); assert.match(ui,/backtest\/long-term\/historical-adjusted\.json/); assert.match(ui,/dates\[0\] !== taipeiToday\(\)/); assert.doesNotMatch(ui,/IRR|XIRR|TWR/); });
check("Phase 2 information architecture and responsive styles exist", () => { for(const id of ["portfolioPerformanceChart","capitalPlanCash"]) assert.match(html,new RegExp(`id="${id}"`)); for(const className of ["portfolioPerformancePanel","portfolioCapitalPlan"]) assert.match(html,new RegExp(`class="[^"]*${className}`)); assert.match(css,/portfolioPerformanceBody/); assert.match(css,/@media\(max-width:430px\)/); });
