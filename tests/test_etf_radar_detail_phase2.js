...
const context={Number,Math,window:{HSDecisionLayerV1:decisionLayer},esc:value=>String(value),officialDisplayScore:row=>{const score=Number(row?.display_score??row?.final_core_score);return Number.isFinite(score)?Math.floor(score):null},finalDecisionBadgeLabel:presentation=>({PULLBACK_WATCH:"回檔觀察",EXTREME_REFERENCE:"極端機會"}[presentation?.decision_stage]||presentation?.decision_label_zh||"資料暫缺")};
vm.createContext(context);vm.runInContext(html.slice(phase1Start,phase1End)+"\n"+html.slice(detailStart,detailEnd),context);
const pair=(latest,previous)=>({latest:{items:{"00830":latest}},previous:{items:{"00830":previous}}}),render=(score,latest,previous,decisionStage="PULLBACK_WATCH")=>context.radarDetailCoreStatusHtml({id:"00830",name:"國泰費城半導體",date:"2026-09-17"},{marketAsOf:"2026-09-17T13:30:00+08:00"},score,pair(latest,previous),{decision_stage:decisionStage},1);
let output=render(45,{display_score:45},{display_score:49});
assert.match(output,/45[\s\S]*CORE SCORE/);assert.match(output,/回檔觀察/);assert.match(output,/49 → 45/);assert.match(output,/▼4/);assert.match(output,/下一級[\s\S]*50 小額加碼/);assert.match(output,/距離[\s\S]*5 分/);
for(const [score,threshold,label,distance] of [[19,40,"回檔觀察",21],[49,50,"小額加碼",1],[50,65,"正式分批",15],[64,65,"正式分批",1],[65,70,"深跌加碼",5],[79,80,"罕見機會",1],[89,90,"極端機會",1]]){output=render(score,{display_score:score},{display_score:score});assert.match(output,new RegExp(`${threshold} ${label}`));assert.match(output,new RegExp(`>${distance} 分<`))}
assert.match(render(90,{display_score:90},{display_score:89},"EXTREME_REFERENCE"),/已進入最高級別/);
assert.match(render(45,{display_score:45},null),/今日變化[\s\S]*>—</);
output=render(null,null,{display_score:49},null);assert.match(output,/資料暫缺/);assert.doesNotMatch(output,/小額加碼|正式分批|深跌加碼/);
