...
console.log("TEST 1 PASS: 09:30 without a finalized baseline shows first-of-day and dash");
...
assert.strictEqual(sameRank.displayRankChange,"排名不變");assert.strictEqual(sameRank.displayScoreChange,"分數 +2.0");
...
assert.strictEqual(down.displayRankChange,"↓2");assert.strictEqual(down.displayScoreChange,"分數 -3.0");
...
console.log("TEST 8 PASS: next-day 09:30 without finalized history does not invent a baseline");
...
assert.strictEqual(newer.snapshots[1].marketAsOf,older.snapshots[1].marketAsOf);
...
console.log("TEST 9 PASS: a successful slot snapshot is immutable and cannot be overwritten");
...
assert.deepStrictEqual(JSON.parse(JSON.stringify(trace1030)),{rankDelta:0,scoreDelta:2,displayRankChange:"排名不變",displayScoreChange:"分數 +2.0"});
assert.deepStrictEqual(JSON.parse(JSON.stringify(trace1130)),{rankDelta:0,scoreDelta:-1,displayRankChange:"排名不變",displayScoreChange:"分數 -1.0"});
console.log("00830 TRACE");
console.log("09:30 score=34 rank=2 previousSnapshot=none displayRankChange=本日首次 displayScoreChange=—");
console.log("10:30 score=36 rank=2 previousSnapshotTime=09:30 previousScore=34 previousRank=2 scoreDelta=+2 rankDelta=0 displayRankChange=排名不變 displayScoreChange=分數 +2.0");
console.log("11:30 score=35 rank=2 previousSnapshotTime=10:30 previousScore=36 previousRank=2 scoreDelta=-1 rankDelta=0 displayRankChange=排名不變 displayScoreChange=分數 -1.0");
