...
async function inputFromOfficialCache(cache,expectedDate,decisionProvider=decisionAtClose){
  const expected=date(expectedDate);if(!expected)fail("missing_expected_trading_date");
  const eod=cache?.official_eod_snapshot,d=date(eod?.date),observedAt=String(eod?.observed_at||"");
  if(!d||eod?.snapshot_type!=="OFFICIAL_CLOSE_INPUT"||eod?.source_status!=="official_closing_data"||!observedAt||!Number.isFinite(Date.parse(observedAt)))notReady("official_eod_snapshot_unavailable");
  ...
  for(const symbol of SCORED){if(date(eod?.items?.[symbol]?.date)!==d||String(eod?.items?.[symbol]?.quote_mode||"")!=="close")notReady(`official_close_input_invalid_${symbol}`)}
  const decisions=Object.fromEntries(await Promise.all(SCORED.map(async symbol=>[symbol,await decisionProvider(symbol,d)])));
  return{date:d,snapshot_type:"FINALIZED_CLOSE",finalized:true,finalized_at:`${d}T13:30:00+08:00`,data_as_of:`${d}T13:30:00+08:00`,decisions};
}
async function main(){
  const args=parse(process.argv),inputPath=args.input?path.resolve(args.input):"";
  const input=inputPath?read(inputPath,null):await inputFromOfficialCache(read(path.resolve(args.market||path.join(ROOT,"market-quotes.json")),null),args["expected-date"]);
...
module.exports={SCORED,VERSION,WEIGHTS,SourceNotReady,buildSnapshot,append,inputFromOfficialCache,decisionAtClose};
