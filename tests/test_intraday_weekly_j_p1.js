...
assert.match(html,/intradayLongRankAsOf\(longs\)/);assert.match(html,/item\.intraday\?\.asOf/);assert.match(html,/isVerifiedLongRankSnapshot\(liveRadarRefresh,candidateSnapshot\)/);assert.match(html,/longRankSnapshot\(rankedLongs,saved,snapshotIdentity,snapshotAsOf,snapshotSlot\)/);assert.match(html,/minute>=30&&minute<=49/);
