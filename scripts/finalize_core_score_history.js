...
async function inputFromOfficialCache(cache){const eod=cache?.official_eod_snapshot,d=date(eod?.date);if(!d||eod?.snapshot_type!=="OFFICIAL_CLOSE_INPUT"||eod?.source_status!=="official_closing_data"||!eod?.observed_at)notReady("official_eod_snapshot_unavailable");for(const symbol of SCORED){if(date(eod?.items?.[symbol]?.date)!==d||String(eod?.items?.[symbol]?.quote_mode||"")!=="close")notReady(`official_close_input_invalid_${symbol}`)}const decisions=Object.fromEntries(await Promise.all(SCORED.map(async symbol=>[symbol,await decisionAtClose(symbol,d)])));return{date:d,snapshot_type:"FINALIZED_CLOSE",finalized:true,finalized_at:`${d}T13:30:00+08:00`,data_as_of:`${d}T13:30:00+08:00`,decisions}}
...
module.exports={SCORED,VERSION,SourceNotReady,buildSnapshot,append,inputFromOfficialCache};
