...
assert.match(html, /distance_to_next_stage/);
assert.match(html, /function finalDecisionPresentation\(item,record=/);
assert.match(html, /function hsLiveDriverSummary\(canonical\)/);
assert.match(html, /function hsLiveTimeline\(symbol,snapshot\)/);
assert.match(html, /async function loadCanonicalCoreLedger\(signal,/);
assert.match(html, /const snapshots=parseCanonicalCoreSnapshots\(payload\)/);
assert.match(html, /applyLiveCoreState\(requestSequence,/);
assert.match(html, /function currentCanonicalCoreSnapshot/);
assert.match(html, /function currentDualTrackCoreView/);
assert.match(html, /function hsLiveProjectedHtml/);
assert.match(html, /正式分數 <i>OFFICIAL<\/i>/);
assert.match(html, /盤中預估 <i>LIVE PROJECTED<\/i>/);
assert.match(html, /盤中參考/);
assert.match(html, /const canonicalForHome=currentCanonicalCoreSnapshot\(taipeiToday\(\)\)/);
assert.match(html, /const snapshotForRender=currentCanonicalCoreSnapshot\(taipeiToday\(\)\)/);
assert.match(html, /canonicalForHome&&!applyCanonicalCoreSnapshot\(canonicalForHome\)/);
...
assert.match(html, /function hsLiveSnapshotFreshness\(snapshot,completeness,today=taipeiToday\(\)\)/);
assert.match(html, /最近成功 \$\{formatHomeMonthDay\(day\)\} \$\{slot\}/);
assert.match(html, /今日盤中資料尚未成功/);
assert.match(html, /function hsLiveCloseComparisonLabel\(canonical,snapshot,today=taipeiToday\(\)\)/);
assert.match(html, /vs \$\{baseline\} 正式收盤/);
